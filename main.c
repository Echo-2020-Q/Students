...
int iStudent_number[31],iMarks[Z+1][7],iSumMarks[Z+1],iNumberofstudents,iNumberofcourses;
char cName[Z+2][30];
...
void DataOutput(char cName[Z+2][30],int iMarks[Z+1][7],int iStudent_number[],int iNumberofstudents)             //*函数功能：输出学生的学号和成绩*/
{
    ...
    {
        //*返回值：无*//
        printf("%-8s",cName[Counter2]);
        printf("\t\t%-8d",iStudent_number[Counter2]);
        for(i=1; i<=iNumberofcourses; i++)
        {
            printf("\t%d\t",iMarks[Counter2][i]);
        }
        Sum_Avermarks(iMarks,Counter2);                                                                         //*根据学生计算总分，均分的函数*//
    }
}
